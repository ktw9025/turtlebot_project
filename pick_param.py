...
class Pick_param(Node):

    def __init__(self):
        super().__init__('pick_param')
        self.call_param    = self.create_timer(1, self.check_param)
        self.param = ''
        self.param2 = ''

    def check_param(self):
        temp = os.popen("ros2 param get /reg_params pick_place").read().strip()
...
def main(args=None):
    ...
    try:
        while rclpy.ok:
            ...
            if param2 == 'stop':
                if param == 'place0':
                    os.system("ros2 run project_3 pick_nav0 ")
                    os.system("ros2 param set /reg_params pick_place place3 ")
                    os.system("ros2 param set /reg_params delivery go ")
                    

                if param == 'place1':
                    os.system("ros2 run project_3 pick_nav1 ")
                    os.system("ros2 param set /reg_params pick_place place3 ")
                    os.system("ros2 param set /reg_params delivery go ")
                    

                if param == 'place2':
                    os.system("ros2 run project_3 pick_nav2 ")
                    os.system("ros2 param set /reg_params pick_place zero ")
                    os.system("ros2 param set /reg_params delivery go ")
