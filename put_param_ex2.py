...
class Check_param(Node):
    ...
    def check_param(self):
        temp1 = os.popen("ros2 param get /reg_params put_goal0").read().strip()
        temp_param1 = temp1[17:].strip()  # 필요한 부분을 잘라내고 공백 제거
        self.param1 = temp_param1
        temp2 = os.popen("ros2 param get /reg_params put_goal1").read().strip()
        temp_param2 = temp2[17:].strip()  # 필요한 부분을 잘라내고 공백 제거
        self.param2 = temp_param2
        temp3 = os.popen("ros2 param get /reg_params put_goal2").read().strip()
...
def main(args=None):
    ...
    try:
        while rclpy.ok():
            rclpy.spin_once(node, timeout_sec=2)  # 콜백 함수가 실행될 때까지 기다림
            ...
            if param4 == 'go':
                if param1 == 'already':
                    os.system("ros2 run project_3 put_track_marker 3 ")
                    os.system("ros2 param set /reg_params put_goal0 yet ")
                    os.system("ros2 param set /reg_params delivery stop ")

                if param2 == 'already':
                    os.system("ros2 run project_3 put_track_marker 4 ")
                    os.system("ros2 param set /reg_params put_goal1 yet ")
                    os.system("ros2 param set /reg_params delivery stop ")

                if param3 == 'already':
                    os.system("ros2 run project_3 put_track_marker 5 ")
                    os.system("ros2 param set /reg_params put_goal2 yet ")
                    os.system("ros2 param set /reg_params delivery stop ")
